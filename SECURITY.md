...
to a remote ref, writes outside the session worktree boundary (for listed file-write tools when CWD is available), writes into `.git` or build output. It is deliberately narrow, it is not a security boundary, and it
