...
diff. `old_base = git merge-base <stage-9-cleared> HEAD`. `new_base = git merge-base HEAD origin/main`, using **only** a freshly force-fetched `origin/main`: the skill force-refreshes it (`git fetch --no-tags --depth=1 origin +refs/heads/main:refs/remotes/origin/main`) and verifies it equals the remote tip (`git ls-remote origin refs/heads/main`) immediately before computing the delta, and if the fetch fails, the ref cannot be resolved, or it is stale against the remote tip it fails closed with **Could not run** / **NEEDS FIXES**. There is no fallback to `origin/master`, a local `main`/`master`, or `git merge-base --fork-point`, and a stale `origin/main` is not accepted — a stale base moves `new_base` backwards and pulls unrelated upstream churn into `new_base..HEAD`. The old series `old_base..stage-9-cleared` and the new series `new_base..HEAD` are compared by patch-id / `git range-diff`: a new commit whose patch-id already exists in the old series is already-cleared feature work and is excluded, and commits that are ancestors of `new_base` (upstream churn) are not in the new series and are excluded. A commit with no patch-id — a merge commit or an empty commit — never matches the old series and is conservatively kept in the filtered delta: it is over-reported, never silently dropped. Empty-delta detection and the non-empty review use the same filtered comparison, and the artifact's diff evidence, filtered commit list, `fixed_point`, and `diff_range` are synchronized to that same scope. The producer and the consumers (`/code-review`'s nested pre-pass and the Security/Coverage re-derivation) use the same fresh-`origin/main`-only rule and fail closed rather than substituting an alternate base.
...
**Accept `origin/master`, a local `main`/`master`, or `git merge-base --fork-point HEAD` as alternate `new_base` sources.** A divergent default-branch source silently changes the delta scope, and a stale `origin/main` moves `new_base` backwards and re-admits upstream churn. The accepted rule is one source — freshly fetched `origin/main` — and fail closed (`Could not run` / `NEEDS FIXES`) when it cannot be resolved, so no alternate is ever silently substituted.
...
named confirmation and does not call `/code-review`. Security and Coverage consumers re-derive the same filtered semantics from a freshly force-fetched `origin/main` only and verify `repository`, `head_sha`, `fixed_point`, and `diff_range` before trusting the ship-review artifact; a consumer that cannot resolve fresh `origin/main`, or finds it stale against the remote tip, fails closed with **Could not run** / **NEEDS FIXES**.
