...
- `--trust-pr` — permit executing PR-provided code, builds, tests, restores, and repository tooling **only inside a credential-free containment boundary** the reviewing host actually provides — a container, VM, or equivalent sandbox. Where the host provides none, execute only after per-run unsandboxed consent from the user, recorded in the coverage ledger. A worktree is checkout isolation, not a sandbox, and never satisfies this flag on its own; see **Trust boundary** below for the full contract.
...
- Only proceed unsandboxed after telling the user plainly that `--trust-pr` on this host grants the PR full access to their credentials, files, and network, and getting explicit confirmation for that run. Silence is not consent, and one confirmation covers one run. Record that consent in the coverage ledger, so a review published from an unsandboxed run says so.
...
**Publication is pinned to a base/head pair.** Both SHAs are re-read immediately before every submission attempt, and either one having moved aborts the run. That check is owned by the submission step itself rather than by its callers, so work accumulating between the run's earlier checks and the POST cannot quietly widen the window the claim describes. The line map is built from `compare/<base>...<head>`, which is addressed by SHA, rather than from the PR's files view, which always describes wherever the PR points right now. Checking only the head left a base-branch advance — which changes what the diff means — undetected.
