...
Then walk the codebase. Delegate this to a **read-only agent** when the host provides one (`code-reviewer` from this harness works — it is `readonly`, so it can range widely without editing), otherwise explore inline. Either way, don't follow rigid heuristics — explore organically and note where you experience friction. The .NET-flavoured versions of the usual suspects:
