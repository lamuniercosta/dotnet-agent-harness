...
- `--trust-pr` alone does not authorize execution. The caller must also tell you which containment boundary applies, or instead pass an explicit record that the user gave per-run unsandboxed consent — naming that it applies to this run. **Without a containment boundary or that recorded consent, do not execute** — produce the static patch, and report the empirical claims as unverified rather than clean.
- When you may not execute, do not check the assigned worktree out. Materialize only the in-scope files by writing the bytes the caller supplies (read via `git cat-file blob <sha>:<path>`) into your worktree directory, edit those, and return the diff — a checkout would run `post-checkout` hooks and PR-selected filter drivers, which is execution.
