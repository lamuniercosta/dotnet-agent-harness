...
The live seat map is Maestri workspace state at `~/.maestri/workspaces/<workspaceId>/seat-map.json` (resolved lazily from the active workspace; an explicit `-SeatMapPath` still overrides). It defines host and tier allocation for team seats as a **schemaVersion 2 declared-rung array** (typical names `head`, `then`, `alt`, `floor`; extra declared names are valid), ordered best first, with a **floor-role** rung marking the lowest option that still does the work without losing quality. `scripts/local/seat-map.example.json` is the schema, the invariants block, and the CI contract.
...
`-Verify` is a **read-only** drift check: it validates the seat map first, then compares each seat's **head-rung** launch (not `activeRung`) to the matching Maestri terminal command in `~/.maestri/workspaces/<workspaceId>/workspace.json` (`payload.nodes[*].content.terminal._0`, matched by seat `roleId` to terminal `assignedRoleId`). Any failure exits **1**. Workspace-level fatals — unresolved workspace, missing or unreadable `workspace.json`, malformed workspace payload, or zero parsed terminal records — stop before seat comparison. Seat-level findings — command drift, missing or duplicate terminal per seat — are collected in `seatMap.seats` order, reported together, then one final exit. Output is redacted — codename, roleId, and SHA-256 hashes only; no full launch commands. `-SeatMapPath` overrides the map; `-WorkspaceId` or repo-root discovery selects the workspace. `-All` keeps its existing write/sync phases, runs verify last, and exits 1 on verify failure after printing `Sync phases completed before verify failure; workspace drift remains.` (no rollback). CI proves the contract through `Test-SeatMapLive.ps1` with an isolated HOME fixture, not a live `~/.maestri` workspace. Running `-Verify` against your real profile is optional operator proof only — not a merge-bar gate.
