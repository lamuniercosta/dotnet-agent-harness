...
- The `guard.ps1` PreToolUse hook now blocks **deleting a protected branch via
  ...
  an empty-source `:dst` refspec (`git push origin :main`,
  `:refs/heads/main`, `+:main`). Detection is per-refspec, so deleting a task
  branch alongside a plain push to a protected branch in one command is still
  allowed, as are fast-forward pushes, `--no-delete`, and `--dry-run`.
